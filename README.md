...
│   Game clients (apps)  │ ───────────▶ │  backend + worker        │ ─────────────────▶ │  ScoreASC (CC3)              │
└────────────────────────┘  (off-chain) │  (off-chain)             │  (Attestcoin proof) │  0x0FD2 Block Prover         │
                                        └──────────────────────────┘                     └──────────────────────────────┘
                                                                                         │ ScoreVerified
                                                                                         ▼
        ┌────────────────────────────────────────────┐   settle   ┌─────────────────────────────────────────────┐
        │ LeaderboardEngine / RoomEngine             │ ─────────▶ │ ArcadeBank + CoinVault + ARCFT token        │
        └────────────────────────────────────────────┘            └─────────────────────────────────────────────┘
```
...
## Full-stack status (Aug 30, 2026)

End-to-end arcade is built, tested, and pushed. **Loop closed** — backend → Sepolia
emission → worker Attestcoin proof → ScoreASC verify → backend reconciliation.
...
| Backend | Done | Full REST API: wallet/economy, leaderboard, rooms (escrow+settle), marketplace, spin, stats, **verify** reconciliation |
...
node scripts/e2e-test.mjs  # backend economy e2e smoke test (33 checks)
```
...
- Backend now **emits GameResultSubmitted** on the Sepolia GameArbiter for every
  ...
  set; live broadcast when armed). See `backend/src/emitSource.ts`.
- Worker relays the **ScoreASC proof-verification** to a dedicated
  ...
  pending (player, gameId, score) to `verified` **without** charging a second
...
### To go fully LIVE (needs funded keys — not deployable without them)
1. **Sepolia**: fund a deployer, run `forge script DeployArcade.s.sol` (or deploy
